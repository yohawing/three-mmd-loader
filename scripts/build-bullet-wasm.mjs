...
const bindings = join(root, "native", "bullet", "ammo_compat_bindings.cc");
const outDir = join(root, "native", "bullet", "dist");
const buildDir = join(outDir, ".tmp", `${process.pid}-${Date.now().toString(36)}`);
const tmpJs = join(buildDir, "yw_bullet_ammo.js");
const tmpWasm = join(buildDir, "yw_bullet_ammo.wasm");
const responseFile = join(buildDir, "emcc-args.rsp");
const outJs = join(outDir, "yw_bullet_ammo.js");
...
async function main() {
  ...
  console.log(`Compiling Bullet Ammo compat build with ${sources.length} sources.`);
