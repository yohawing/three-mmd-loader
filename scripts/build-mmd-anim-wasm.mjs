...
const artifactRoot = join(root, "artifacts", "mmd-anim", "runtime");
const pkgDir = join(artifactRoot, "pkg");
const artifacts = [
  "mmd_anim_wasm.js",
...
async function main() {
  ...
  await spawnCommand(
    "wasm-pack",
    ["build", "--target", "web", "--out-dir", relative(crateDir, pkgDir)],
    crateDir
  );
  await copyArtifacts(pkgDir, artifactRoot);
  console.log(`mmd-anim-wasm build complete: ${artifactRoot}`);
}
