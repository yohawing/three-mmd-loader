...
describe("example viewer source", () => {
  ...
  it("compiles view-toggle shader variants asynchronously without idle prewarm (T070-19)", async () => {
    ...
    expect(debugSource).not.toContain("shaderPrewarmToken");
  ...
  it("wires the main viewer as a TSL parity review viewer with a baseline fallback", async () => {
    ...
    const selfShadowAnalysisSource = await readFile("scripts/visual-regression/viewer-self-shadow-analysis.mjs", "utf8");
    const selfShadowGateContracts = `${selfShadowGateSource}\n${selfShadowAnalysisSource}`;
    ...
    expect(selfShadowGateSource).toContain('from "./viewer-self-shadow-analysis.mjs"');
    expect(selfShadowAnalysisSource).toContain("export function analyzeReceiverDarkening");
    expect(selfShadowAnalysisSource).toContain("export function selfShadowDiagnosticsPass");
    expect(selfShadowGateContracts).toContain('options.backend === "baseline"');
    expect(selfShadowGateContracts).toContain('backend === "webgpu"');
    expect(selfShadowGateContracts).toContain('options.backend = value.toLowerCase()');
    expect(selfShadowGateContracts).toContain("--dedicated-raw-visibility requires --backend webgpu.");
    expect(selfShadowGateContracts).toContain("renderer?.isWebGLRenderer === true");
    expect(selfShadowGateContracts).toContain("renderer?.isWebGPURenderer === true");
    expect(selfShadowGateContracts).toContain("typeof renderer.renderAsync === \"function\"");
    expect(selfShadowGateContracts).toContain("--raw-visibility requires a TSL backend");
    expect(selfShadowGateContracts).toContain("--vmd-lifecycle");
    expect(selfShadowGateContracts).toContain("vmdLifecyclePixelPass(fullFrame");
    expect(selfShadowGateContracts).toContain("vmdInactiveMeanDarkeningMax");
    expect(selfShadowGateContracts).toContain("mode === 0");
    expect(selfShadowGateContracts).toContain("mode === 1 || mode === 2");
    expect(selfShadowGateContracts).toContain("vmdObservation(primary.on.diagnostics)");
    expect(selfShadowGateContracts).toContain('status: "not-applicable"');
    expect(selfShadowGateContracts).toContain("VMD self-shadow mode disabled");
    expect(selfShadowGateContracts).toContain("onModeDistanceAgreement");
    expect(selfShadowGateContracts).toContain("localFullFrameP995DarkeningMin");
    expect(selfShadowGateContracts).toContain("localBackgroundMaxDarkeningMin");
    expect(selfShadowGateContracts).toContain("maxDarkening");
    expect(selfShadowGateContracts).not.toContain("Math.max(...darkening");
    expect(selfShadowGateContracts).toContain("metrics.primary.p995Darkening >= p995Min");
    expect(selfShadowGateContracts).toContain("p95 remains diagnostic-only");
    expect(selfShadowGateContracts).toContain("const inactiveVmdMode = options.vmdLifecycle");
    expect(selfShadowGateContracts).toContain("requireCastShadow = true");
    expect(selfShadowGateContracts).toContain("shadowCameraFar");
    expect(selfShadowGateContracts).toContain("vmdShadowCameraFarPass");
    expect(selfShadowGateContracts).toContain("Math.min(Math.max(on.distance * 100, off.shadowCameraFar), 100)");
    expect(selfShadowGateContracts).toContain("const onShadowCameraFarPass = !enabled");
  ...
  it("keeps the independent main-viewer native WebGPU background visual gate wired to its synthetic fixture", async () => {
    ...
    expect(gateSource).toContain('page.off("pageerror", baselinePageErrorListener)');
    expect(gateSource).toContain("const pageErrorListener = error => messages.push(error.message);");
