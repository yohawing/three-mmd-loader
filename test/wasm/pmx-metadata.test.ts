...
describe("@yw-mmd/core-wasm PMX metadata", () => {
  ...
  it("requires the split PMX ABI instead of using the old full-JSON fallback", () => {
    const core = new MmdAnimBackedCore({
      wasm_wrapper_version: () => 1
    });

    expect(() => core.loadModel(createMinimalSdefPmx(), { format: "pmx" })).toThrow(
      /PMX split ABI is required/
    );
  });
  ...
  it("rejects broken PMX data", async () => {
    ...
    expect(() => core.loadModel(new Uint8Array([0, 1, 2, 3]), { format: "pmx" })).toThrow(
      /Invalid PMX signature|Unexpected end|invalid PMX magic bytes/
    );
  });
