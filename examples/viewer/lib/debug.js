...
export function createViewerDebugApi() {
  return {
    async showNormals() {
      await setDebugMaterialMode("normals");
    ...
    async selfShadow(enabled = true) {
      return `selfShadow=${await setSelfShadowEnabled(enabled)}`;
    },
...
export async function setDebugMaterialMode(mode) {
  ...
  refreshDebugPanelState();
  // The material swap above already reflects in state.debugMaterialMode; only
  // the (possibly shader-compiling) render is deferred until it's ready, so
  // the UI/checkbox stays responsive during the async compile (T070-19).
...
export async function setSelfShadowEnabled(enabled) {
  ...
  syncMmdTslDedicatedRawVisibilityDebug();
  refreshDebugPanelState();
  // Flipping shadowMap.enabled/castShadow changes every visible material's
  // shadow macro permutation; defer the recompile off the synchronous path
